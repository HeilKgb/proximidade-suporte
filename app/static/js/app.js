...
app.constant('PROX_SUPORTE_URLS', {
...
app.run(['$rootScope', '$state', '$transitions', '$window', 'AUTH_EVENTS', 'PROX_SUPORTE_URLS',
function ($rootScope, $state, $transitions, $window, AUTH_EVENTS, PROX_SUPORTE_URLS) {
	...
	$transitions.onBefore({}, function(transition) {
		...
		AuthService.CheckAuthenticated().then(function(response){
			...
			$window.open(PROX_SUPORTE_URLS.unauthenticated, '_self');
			return false;
		});
	});
...
app.config(['$stateProvider', '$urlRouterProvider', 'USER_ROLES', 'PROX_SUPORTE_URLS',
function ($stateProvider, $urlRouterProvider, USER_ROLES, PROX_SUPORTE_URLS) {
		...
		templateProvider: ['$timeout', function ($timeout) {
			return $timeout(function () {
				return '<p class="lead">Suporte PROX</p><ul>' +
					   '<li><a href="' + PROX_SUPORTE_URLS.main_app + '">Suporte PROX Web Site</a></li>' +
					   '</ul>';
			}, 100);
		}],
		data: {
